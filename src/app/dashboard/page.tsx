  ...
  return (
    <div className="rounded-[22px] bg-white p-6 shadow-sm ring-1 ring-black/5">
      <div className="flex items-center gap-2 text-[22px] font-bold text-black">
        <span>{label}</span>
        <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-gray-300 text-[12px] leading-none text-gray-400">?</span>
      </div>
      <div className="mt-6 flex items-baseline gap-2">
        ...
        {unit && <span className="text-[18px] font-semibold text-gray-900">{unit}</span>}
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  return (
    ...
    >
      {/* 헤더 */}
      <header className="mb-1 px-4 pt-6 text-center">
        <h1 className="bg-gradient-to-r from-violet-600 to-sky-500 bg-clip-text text-[22px] font-extrabold leading-tight text-transparent">
          나무의 상태
        </h1>
        <p className="bg-gradient-to-r from-violet-600 to-sky-500 bg-clip-text text-[22px] font-extrabold leading-tight text-transparent">
          분석이 완료되었어요
        </p>
      </header>
      ...
      <main className="flex-1 px-4 pb-28">
        ...
        <section className="mb-3">
          <div className="mb-2 text-[18px] font-extrabold text-gray-900">AI 요약</div>
          <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-2">
            <SummaryCard title="전체 상태" body="지금 숲은 잘 자라고 있어요." />
            <SummaryCard title="성장 추세" body="잎이 차오르고 줄기가 서서히 굵어지고 있어요." />
            <SummaryCard title="어린 나무" body="아직 체력이 약해 바람·건조에 민감해요. 보호가 필요합니다." />
            <SummaryCard title="다음 행동" body="번 달엔 보식 몇 그루 + 물주기 강화가 가장 효과적이에요." />
          </div>
        </section>
